...
void User::request_trip(int origin_index, int destination_indexint, int user_index, vector<Trip> &trips, int &trips_num,double price)
{
...
void User::check_costs_accessability()
{
    throw Error(PERMISSION_DENIED);
}
...
void Passenger::request_trip(int origin_index, int destination_index, int user_index, vector<Trip> &trips, int &trips_num,double price)
{
    if (is_travelling)
    {
    ...
    {
        trips_num++;
        Trip new_trip(user_index, origin_index, destination_index, username, trips_num,price);
